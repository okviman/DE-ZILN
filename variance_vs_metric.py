...
_, (sc_ax, DELN_ax) = plt.subplots(1, 2, figsize=(30, 15), sharey=True)
...
for i, d1 in enumerate(d1_list):
    ...
    for d2 in d2_list:
        ...
        if np.sum(DELN_adj_pvals >= 0.05) == n_genes:
            ...
            deln_results = get_test_results(DELN_adj_pvals, true_lfcs, verbose=False)

        results["sc"] += [sc_results[metric]]
        results["DELN"] += [deln_results[metric]]
    sc_ax.plot(var_2.astype(int), results["sc"], lw=5)
    DELN_ax.plot(var_2.astype(int), results["DELN"], label={f"Var$(X$)={int(var_1[i])}"}, lw=5)
